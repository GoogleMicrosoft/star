...
std::unique_ptr<Node> KDTreeIndex::InsertHelper(Node* node,
...
Node* KDTreeIndex::NearestNeighborHelper(Node* node,
...
Status KDTreeIndex::Add(const AddRequest& request) {
  for (const auto& x : request.vecs) {
    root_ = InsertHelper(root_.get(), x.vec.x, 0);
  }
  return OkStatus();
}
